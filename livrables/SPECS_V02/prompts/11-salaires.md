...
- import prévisualisable des taux annuels depuis la table Lasso
  `taux_par_annee` de la base indiquée par `APP_DB_PATH` ;
...
Les données Lasso sont prioritaires pour la reprise, mais restent datées,
sourcées et validées par l'opérateur. Si la base Lasso n'est pas disponible,
...
servi à une fiche validée. Préserve les 32 calculs de parité Lasso et tous les
snapshots.

Acceptation :

- prévisualisation sans écriture, confirmation auditée et rejeu idempotent ;
- chaque année/clé Lasso est importée ou justifiée comme non applicable ;
